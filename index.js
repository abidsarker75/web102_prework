...
function addGamesToPage(games) {
  ...
  for (let i = 0; i < games.length; i++) {
    ...
    div.innerHTML = `<img class="game-img" src="${games[i].img}" alt="${games[i].name}" />
  <h3>${games[i].name}</h3>
  ...
  <p>Backers: ${games[i].backers}</p>}`;
    ...
    const gameCont = document.getElementById("games-container");
    gameCont.append(div);
  }
}
...
function filterUnfundedOnly() {
  ...
  const unfunded = GAMES_JSON.filter(
    (games) => games["pledged"] < games["goal"]
  );
...
function filterFundedOnly() {
  ...
  const funded = GAMES_JSON.filter(
    (games) => games["pledged"] >= games["goal"]
  );
...
const unfundedMessage =
  unfundedGames === 1
    ...
    : `There are ${unfundedGames} unfunded games.`;
// create a new DOM element containing the template string and append it to the description container
const unfundedParagraph = document.createElement("p");
unfundedParagraph.innerHTML = unfundedMessage;
